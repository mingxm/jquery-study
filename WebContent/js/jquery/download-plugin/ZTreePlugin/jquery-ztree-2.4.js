...
(function($) {
	...
	$.fn.zTree = function(zTreeSetting, zTreeNodes) {
		...
		if (zTreeSetting) {
			...
			//extend是合并对象的函数，将总TreeSetting合并到json对象setting之中，后者将会覆盖前者中相同的名的值为后者的值
			//貌似只能拷贝两层，第三层就无法
		...
		//如果root中的存储节点的数组不存在的话，那么设置该节点的数组为空数组
		if (!setting.root[setting.nodesCol]) setting.root[setting.nodesCol]= [];
		zTreeId = 0;

		if (zTreeNodes) {
		//如果传过来有节点数据的话，那么设置到json的root变量之中
			setting.root[setting.nodesCol] = zTreeNodes;
		}
		//isSimpleData是标识是否是简单的普通数组形式还是复杂的json对象形式，如果是简单的数组形式，那么需要将其转换为jason对象的形式
	...
	//绑定事件,就是调用用户自定义的回调函数，这里还是很有技巧性的
